...
| 任务 | 模型 | 文件 | 说明 |
| ---- | ---- | ---- | ---- |
| Task 1 | HMM | `hmm_ner.py` | 监督估计初始、转移、发射概率，手写 Viterbi 解码 |
...
CRF 在中英文上都取得最高 F1，说明在当前数据规模下，强人工特征和显式转移建模仍然最稳定。HMM 作为生成式基线能够学习标签转移，但发射概率只记忆 token，遇到未登录词和复杂上下文时明显受限。
...
2. 改用 BERT/BiLSTM-CRF：在小数据 NER 上，预训练编码器 + CRF 通常比随机初始化 Transformer 更可靠。
