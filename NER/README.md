...
- English：验证集 Ensemble F1 = `0.9065`
- Chinese：验证集 Transformer+CRF F1 = `0.9531`
...
验证集上关闭预训练后 English / Chinese 分别下降 `0.0030` / `0.0049`。把这一版（关闭预训练）的 Trans+CRF 预测灌进 Ensemble，验证集得到 English `0.9053` / Chinese `0.9513`，两个语言都比默认 Ensemble（`0.9065` / `0.9530`）略低，说明预训练对融合器同样有正贡献。
...
验证集英文最优是 Ensemble（F1 `0.9065`），中文最优是 Transformer+CRF（F1 `0.9531`，首次反超 CRF）。三个模型的偏置不同：HMM 依赖生成式统计和 OOV backoff，CRF 依赖人工局部特征，Transformer+CRF 依赖上下文表示和预训练向量。验证集排名反映了这些模型对开发集分布的适配程度。
...
Transformer+CRF 经强化后改善显著：英文验证集 F1 从 `0.8340` 提升到 `0.8944`（+0.0604），中文从 `0.9433` 提升到 `0.9531`（+0.0098）。除了 constrained CRF、按 F1 早停、AdamW/weight decay、word dropout、多卷积核 char-CNN 等优化外，本轮新增的 GloVe / fastText 静态预训练向量（frozen + concat）是关键贡献之一。预训练向量的具体增益由"当前实验结果 → Ablation：关闭预训练词向量"小节量化（验证集 +`0.0030` / +`0.0049`）。
